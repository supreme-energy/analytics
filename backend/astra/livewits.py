...
def connect(rig_id):
    ...
    <soap:Body soap:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/">
      <q1:WMLS_GetFromStore xmlns:q1="http://www.witsml.org/message/120">
        <WMLtypeIn xsi:type="xsd:string">rig</WMLtypeIn>
        <QueryIn xsi:type="xsd:string">&lt;rigs xmlns="http://www.witsml.org/schemas/131" version="1.3.1.1"&gt;
    &lt;rig uid="%s"&gt;
      &lt;nameWell /&gt;
      &lt;nameWellbore /&gt;
      &lt;name /&gt;
    ...
    response = requests.post(
        url, data=main_body, headers=headers, auth=(username, password))

    root = ET.fromstring(response.text)

    xml_content = ET.fromstring(root.find(".//XMLout").text)

    rig_data = xml_content.find(".//{http://www.witsml.org/schemas/131}rig")
    well_name = xml_content.find(
        ".//{http://www.witsml.org/schemas/131}nameWell").text
    rig_name = xml_content.find(
        ".//{http://www.witsml.org/schemas/131}name").text

    uidWell = rig_data.attrib['uidWell']
    uidWellbore = rig_data.attrib['uidWellbore']

    return (url, headers, username, password, uidWell, uidWellbore, well_name, rig_name)


def edrdata(rig_id, date, data_frequency):
    ...
    rig_body = """
      ...
      &lt;log uidWell="%s" uidWellbore="%s" uid="Time_%sSec"&gt;
        &lt;nameWell /&gt;
        &lt;nameWellbore /&gt;
        &lt;name /&gt;
        &lt;objectGrowing /&gt;
    ...
    """ % (uidWell, uidWellbore, data_frequency, date)
